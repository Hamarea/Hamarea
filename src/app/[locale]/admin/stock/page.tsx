...
type InvRow = {
  ...
  product_variants: { sku: string; product_id: string };
};
...
export default async function AdminStockPage() {
  ...
  try {
    const { data } = await (supabase as unknown as {
      ...
      .from("inventory")
      .select("variant_id, warehouse_id, quantity, reserved, reorder_point, product_variants(sku, product_id)");
    rows = data ?? [];
  } catch {
    rows = [];
  }
                ...
                return (
                  <tr key={r.variant_id} className="border-b border-[var(--color-border)]">
                    <td className="px-4 py-3 font-mono text-xs">
                      {r.product_variants?.sku ?? r.variant_id}
                    </td>
